...
data = pickle.load(open(constants.FACE_DATA_VECTOR_PATH, 'rb'))
le = pickle.load(open(constants.FACE_DATA_LABEL_PATH, 'rb'))
...
def camera_open():
    ...
    model = load_model(constants.FACE_MODEL_PATH)
    detector = MTCNN()
    ...
    video_out = cv2.VideoWriter(constants.APPLICATION_OUTPUT_VIDEO_PATH, cv2.VideoWriter_fourcc('M','J','P','G'), 30, (save_width,save_height))
